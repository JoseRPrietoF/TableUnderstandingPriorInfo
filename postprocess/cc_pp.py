...
def print_BBs(nodes, labels, cc,
              results_gt, file_name, dir_to_save, drawing,):
    radius = 10
    color = (255, 0, 0)
    shape = drawing.shape
    ...
    show_cells(drawing,  title=file_name + "_{}_cols".format(len(cc)), fname=file_name, dir_to_save=dir_to_save)
...
def main():
    ...
    results = "/home/jprieto/projects/TableUnderstanding/works/works_conjugate_COL/work_graph_COL_PI_0.15_8000epochs_16,16,16,16,16,16,16ngfs_base_1_zdepth_lentext_probs/results.txt"
    dir_img = "/data/READ_ABP_TABLE/dataset111/img/"
    file_list = get_all(data_path)
    if results == "cols":
        dir_to_save = os.path.join(data_path, dir_save)
    elif results != "no":
        results = os.path.abspath(results)
        dir_to_save = "/".join(results.split("/")[:-1])
        dir_to_save = os.path.join(dir_to_save, dir_save)
        results = read_results(results)
        print("A total of {} lines classifieds in results".format(len(results)))
    else:
        dir_to_save = os.path.join(results, dir_save)
    #TODO borrar
    ...
    for raw_path in file_list:
        if "0010_S_Vilshofen" not in raw_path:
            continue
        ...
        # weighted_edges_all = [(i,j,results.get("{}_edge{}".format(file_name, count))[0]) for count, (i,j) in enumerate(edges)]
        # print(raw_path)
        # print(len(weighted_edges_all))
        # weighted_edges = [(i,j,w) for i,j,w in weighted_edges_all if w > min_w]
        ...
        mst = nx.algorithms.community.k_clique_communities(G, 3)
        mst = list(mst)
        cc2 = []
        # for i in mst[1]:
        for i in mst:
        ...
        count_fail_, words_failed_ = print_BBs(nodes, labels, cc2,
                                               gt_edges_graph_dict, file_name, dir_to_save, img)
        # exit()
