...
def print_BBs(mst,
              file_name, dir_to_save, drawing, ant_nodes):
...
def main():
    ...
    # results = sys.argv[2]
    # dir_img = sys.argv[3]
    dir_res = "minimal_spanning_tree"
    data_path = "/data/READ_ABP_TABLE/dataset111/graphs_structure/graphs_structure_PI_0.15_ZDEPTH/"
    results = "/home/jprieto/projects/TableUnderstanding/works/works_conjugate_COL/work_graph_COL_PI_0.15_8000epochs_16,16,16ngfs_base_1_model_EdgeFeatsConv_root/results.txt"
    dir_img = "/data/READ_ABP_TABLE/dataset111/img/"
    file_list = get_all(data_path)
    if results == "cols":
        dir_to_save = os.path.join(data_path,dir_res )
    elif results != "no":
        results = os.path.abspath(results)
        dir_to_save = "/".join(results.split("/")[:-1])
        dir_to_save = os.path.join(dir_to_save, dir_res)
    ...
    for raw_path in file_list:
        ...
        for count, (i, j) in enumerate(edges):
            ...
            if hyp_prob > 0.5:
                G.add_edge(i, j,weight= 1-hyp_prob, taip = 0)
                G.add_edge(i, j, weight = hyp_prob, taip = 1)
            # if hyp_prob > min_w:
        ...
        e = mst.edges(data=True)
        e_g = G.edges(data=True)
        print(e_g)
        print(e)
        zeros = 0
        for i, j, d in e_g:
            tipe = d['taip']
            zeros += tipe
        print("A total of {} one types from a total of {} edges G".format(zeros, len(e_g)))
        zeros = 0
        for i,j,d in e:
            tipe = d['taip']
            zeros += tipe
            if d['weight'] > 0.5:
                print("Cagada")
        print("A total of {} one types from a total of {} edges".format(zeros, len(e)))
        # exit()
        # print(type(mst))
        # print(len(mst.edges))
        # exit()
        ...
        count_fail_, words_failed_ = print_BBs(mst,
                                               file_name, dir_to_save, img, ant_nodes)
        # exit()
