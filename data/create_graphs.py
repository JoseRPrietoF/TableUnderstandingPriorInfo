...
if len(sys.argv) < 10:
    print("Order: create_graphs.py data_path(PAGE-XML) dir_dest min_cell min_num_neighrbors weight_radius_w weight_radius_h j_h h_w max_width_line [prod]")
...
max_width_line = float(sys.argv[9])
prod = sys.argv[11].lower() in ["true", "si", "1"]
weights = [float(sys.argv[5]),float(sys.argv[6])] # used for radius
hisclima = sys.argv[10].lower() in ["true", "si", "1"]
data_path_tablelabel = sys.argv[14]
try:
    data_type = sys.argv[12].lower() 
except:
    data_type = "textlines"
try:
    info_attributes = sys.argv[13].lower() in ["true", "si", "1"]
except:
...
def main():
    ...
    for i_fname, fname in enumerate(fnames):
        ...
        # page = TablePAGE(im_path=fname, hisClima=hisclima, info_attributes=info_attributes)
        if hisclima and not info_attributes:
            IoU = True
            xml_path_gt = os.path.join(data_path_tablelabel, fname.split("/")[-1])
            page_table = TablePAGE(xml_path_gt, hisClima=True)
            tls_GT = page_table.get_textLinesFromCell()
        else:
            IoU = False
            tls_GT = []
        page = TablePAGE(im_path=fname, hisClima=hisclima, info_attributes=False, IoU=IoU, IoU_tl=tls_GT, IoU_path=xml_path_gt)
        
        
        cells, cell_by_col, cell_by_row = page.get_cells()
        maxWidth = get_max_width_cells(cells, min_num_cells=minWidthNumCells) # TODO redo
        if data_type == "textlines":
            if hisclima:
                BLs = page.get_textLinesHisClima(prod=prod, info_attributes=info_attributes)
            else:
                BLs = page.get_textLinesFromCell(prod=prod, info_attributes=info_attributes)
        elif data_type == "tablecell":
            if hisclima:
                BLs = page.get_cellsWithTextHisClima(prod=prod)
            else:
                BLs = page.get_cellsWithText(prod=prod)
        else:
            raise Exception(f'{data_type} not implemented')
        # print(BLs)
        # exit()
        ids = []
        ids_tl = []
