...
if len(sys.argv) < 10:
    print("Order: create_graphs.py data_path(PAGE-XML) dir_dest min_cell min_num_neighrbors weight_radius_w weight_radius_h j_h h_w max_width_line")
...
def main():
    ...
    fname_search = "vol003_021_0"
    count_total_BLs_table = 0
    total_cells = 0
    for i_fname, fname in enumerate(fnames):
        count_total_BLs_table_i = 0
        if "doc" in fname:
            continue
        ...
        page = TablePAGE(im_path=fname, hisClima=hisclima)
        cells, cell_by_col, cell_by_row = page.get_cells()
        maxWidth = get_max_width_cells(cells, min_num_cells=minWidthNumCells) # TODO redo
        cells_text = page.get_cellsWithText()
        total_cells += len(cells_text)
        print(f'{fname} -> {len(cells_text)}')
        continue
        if hisclima:
            BLs = page.get_textLinesHisClima()
        else:
        ...
        for i, (bl, text, textLine_id, info) in enumerate(BLs):
            ...
            # if row == -1 or col == -1:
            #     continue
            count_total_BLs_table_i += 1
            
        count_total_BLs_table += count_total_BLs_table_i
    print("A total of {} BLs for in tables".format(count_total_BLs_table))
    print(f'A total of {total_cells} cells')
