...
try:
    from conjugate import conjugate_nx, conjugate_nx_spans
except:
    from data.conjugate import conjugate_nx, conjugate_nx_spans
...
dir_orig = sys.argv[1]
dir_dest = sys.argv[2]
only_spans = len(sys.argv) >= 4 and sys.argv[3].lower() in ['true', 'si']
print(sys.argv)
prod = sys.argv[4].lower() in ['true', 'si']
# dir_orig = ""
...
for fname in fnames:
    ...
    # print(f' nodes {len(nodes)}, only_spans {only_spans} - prod {prod}')
    if only_spans:
        if nodes:
            if not prod:
                gts_span = data_load['gts_span']
                ids, new_nodes, new_edges, new_labels, new_edge_feats, ant_nodes, ant_edges = conjugate_nx_spans(ids,
                                nodes, edges, labels, edge_features, gts_span)
                data_load['conjugate'] = ( ids, new_nodes, new_edges, new_labels, new_edge_feats, ant_nodes, ant_edges)
            else:
                gts_span = data_load['gts_span']
                ids, new_nodes, new_edges, new_edge_feats, ant_nodes, ant_edges = conjugate_nx_spans(ids,
                                nodes, edges, labels, edge_features, gts_span)
                data_load['conjugate'] = ( ids, new_nodes, new_edges, new_edge_feats, ant_nodes, ant_edges)
        else:
            data_load['conjugate'] = ( ids, nodes, edges, [], edge_features, nodes, edges)
    else:
        if nodes:
            if not prod:
                ...
                ids, new_nodes, new_edges, new_edge_feats, ant_nodes, ant_edges = conjugate_nx(ids,
                            nodes, edges, labels, edge_features)
            # data_load['conjugate'] = ( ids, new_nodes, new_edges, new_edge_feats, ant_nodes, ant_edges)
        else:
            data_load['conjugate'] = ( ids, nodes, edges, [], [], [], edge_features, nodes, edges)
    # print(f'len ----> {len(data_load["conjugate"])}')
