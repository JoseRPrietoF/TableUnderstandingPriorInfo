...
def conjugate_nx(ids, nodes, edges, labels, edge_features, idx=0, list_idx=[]):
    ...
    if not ids:
        ...
        return new_ids, new_nodes, new_edges, \
           new_edge_feats, list(L.nodes()), list(L.edges())#, follow_edges
...
# @timing
def conjugate_nx_spans(ids, nodes, edges, labels, edge_features, gts_span, idx=0, list_idx=[]):
    prod = labels == []
    ...
    if not ids:
        if not prod:
            return new_ids, new_nodes, new_edges, new_labels, \
            new_edge_feats, list(L.nodes()), list(L.edges())#, follow_edges
        else:
            return new_ids, new_nodes, new_edges, \
           new_edge_feats, list(L.nodes()), list(L.edges())#, follow_edges
    ...
    for num_node, (i, j) in enumerate(L.nodes):
        ...
        if not prod:
            ...
            # gts_j = gts_span.get((row_j, col_j))
            gts_i = gts_span_dict.get((row_i, col_i))
            gts_j = gts_span_dict.get((row_j, col_j))
            
            # if [row_j, col_j] in gts_i or [row_i, col_i] in gts_j or (row_j == row_i and col_j == i):
            if gts_i == gts_j:
                new_labels.append(1)
            else:
                new_labels.append(0)
            # print(f'row_i {row_i} col_i {col_i} - row_j {row_j} col_j {col_j} label {new_labels[-1]}')
