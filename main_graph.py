...
def main():
    ...
    if not opts.fold_paths:
        logger.info("not opts.fold_paths")
        random.shuffle(list_files)
        splits = chunkIt(list_files, 4)
    if opts.test_lst and opts.train_lst:
        logger.info("train and test predefined")
        splits = get_trainTest(opts)
    else:
        if opts.myfolds:
            ...
            logger.info("Not Using our folds")
...
def save_results_prod(results, dir, logger):
    create_dir(dir)
    fname = os.path.join(dir, "results_prod.txt")
    logger.info("Saving results on {}".format(fname))
    f = open(fname, "w")
    f.write("ID PREDICTION\n")
    for id_line, prediction in results:
        f.write("{} {}\n".format(id_line, prediction))
    f.close()
...
def train(list_train_val, list_te, opts, device, logger, writer, transform=None, pre_transform=None, show_nn=False, nfold=1):
    ...
    elif opts.classify == "spans":
        from data.graph_dataset_spans import ABPDataset_BIESO as ABPDataset
        logger.info("Classifying SPANS")
    else:
    ...
    if opts.do_val:
        ...
        dataset_dev = ABPDataset(root=opts.data_path, split="dev", flist=list_val, transform=transform,
                                 pre_transform=pre_transform, opts=opts, dict_layouts=dataset_tr.dict_layouts)
        dataset_dev.process()
    ...
    test_dataset = ABPDataset(root=opts.data_path, split="test", flist=list_te, pre_transform=pre_transform,
                                 transform=None,
                                 opts=opts, dict_layouts=dataset_tr.dict_layouts)
    # test_dataloader = ABPDataset_BIESO(root=opts.data_path, split="test", flist=list_tr, transform=transform, opts=opts)
    test_dataset.process()
    logger.info("A total of {} labels in test dataset".format(len(test_dataset.labels)))
    ...
    if opts.g_loss == "NLL":
        logger.info("Class weight : {} -> {}".format(c_weights, len(c_weights)))
    ...
    for epoch in range(init_epoch, opts.epochs):
        ...
        for batch, sample in enumerate(train_dataloader):
            ...
            y = sample.y
            # l = list(zip(np.argmax(tensor_to_numpy(sample.x), axis=1), tensor_to_numpy(y)))
            # # print(l)
            # if not all([e[0] == e[1] for e in l]):
            #     print(l, batch)
            #     exit()
            # print(y.shape)
            loss = lossFunc(hyp, y)
        ...
        if opts.show_train != -1 and epoch % opts.show_train == 0:
            net.eval()
            predictions, res_gt = [], []
            total = 0
            for v_batch, v_sample in enumerate(train_dataloader):
                ...
                total += len(y_gt)
                predictions.extend(hyp)
                res_gt.append(y_gt)

            # predictions = np.hstack(res_hyp)
            labels = np.hstack(res_gt)
            # labels_fixed = np.hstack(labels_fixed)

            print("A total of {} labels predicted".format(total))
            acc, p, r, f1 = metrics.eval_graph(labels, predictions)
            logger.info("-" * 10 + "TRAIN RESULTS SUMMARY --- EPOCH {} ".format(epoch) + "-" * 10)
            logger.info("Fname - Accuracy - Precision - Recall - F1")
            logger.info("Mean Acc on train epoch {} : {}".format(epoch, acc))
            logger.info("Mean Precision on train epoch {}  : {}".format(epoch, p))
            logger.info("Mean Recal on train epoch {} : {}".format(epoch, r))
            logger.info("Mean F1 on train  epoch {}  : {}".format(epoch, f1))
            if opts.classify == "HEADER":
                pass
            else:
                results_test = list(zip(dataset_tr.ids, labels, predictions))
                # print(results_test)
                fP, fR, fF, res = evaluate_graph_IoU(list_train_val, results_test, min_w=opts.min_prob, th=0.8, type_=opts.conjugate, pruned=opts.do_prune)
                fP_1, fR_1, fF_1, res_1 = evaluate_graph_IoU(list_train_val, results_test, min_w=opts.min_prob, th=1.0, type_=opts.conjugate, pruned=opts.do_prune)
                ...
                logger.info("Mean F1 on train  epoch {}  : {}".format(epoch, f1))
                logger.info("#####   IoU and alignment of connected components  #####")
                logger.info("Mean Precision IoU on train : {}".format(fP))
                logger.info("Mean Recal IoU on train : {}".format(fR))
                logger.info("Mean F1 IoU on train : {}".format(fF))
        ...
        if opts.show_test != -1 and epoch % opts.show_test == 0:
            ...
            for v_batch, v_sample in enumerate(test_dataloader):
                ...
                hyp = tensor_to_numpy(hyp)
                y_gt = tensor_to_numpy(v_sample.y)
                # labels_fixed = tensor_to_numpy(v_sample.labels_fixed)
                ...
                predictions.extend(hyp)
                res_gt.append(y_gt)
                # labels_fixed.append(labels_fixed)
            ...
            if opts.classify == "HEADER":
                ...
                # print(results_test)
                fP, fR, fF, res = evaluate_graph_IoU(list_te, results_test, min_w=opts.min_prob, th=0.8, type_=opts.conjugate, pruned=opts.do_prune)
                fP_1, fR_1, fF_1, res_1 = evaluate_graph_IoU(list_te, results_test, min_w=opts.min_prob, th=1.0, type_=opts.conjugate, pruned=opts.do_prune)
    ...
    with torch.no_grad():
        ...
        if opts.classify == "HEADER":
            ...
            fP, fR, fF, res = evaluate_graph_IoU(list_train_val, results_train, min_w=opts.min_prob, th=0.8, type_=opts.conjugate, pruned=opts.do_prune)
            fP_1, fR_1, fF_1, res_1 = evaluate_graph_IoU(list_train_val, results_train, min_w=opts.min_prob, th=1.0, type_=opts.conjugate, pruned=opts.do_prune)
            logger.info("#####   IoU and alignment of connected components  #####")
    ...
    with torch.no_grad():
        ...
        labels = np.hstack(res_gt)
        logger.info("A total of {} labels predicted".format(total))
    ...
    if opts.do_prod:
        ...
        list_prod = get_all(opts.prod_data)
        # print(list_prod)
        # exit()
        prod_dataset = ABPDataset(root=opts.prod_data, split="prod", flist=list_prod, pre_transform=pre_transform,
                                 transform=None,
                                 opts=opts)
        # test_dataloader = ABPDataset_BIESO(root=opts.data_path, split="test", flist=list_tr, transform=transform, opts=opts)
        prod_dataset.process()
        ...
        with torch.no_grad():
            ...
            predictions = []
            net.eval()
            for v_batch, v_sample in enumerate(prod_dataloader):
                # f_names = v_sample.fname
                hyp = net(v_sample.to(device))
                # _, hyp = torch.max(hyp, dim=1)
                hyp = tensor_to_numpy(hyp)
            ...
            results_prod = list(zip(prod_dataset.ids, predictions))
            

            test_end_time = time.time()
            save_results_prod(results_prod, opts.work_dir, logger)
